...
description: >-
  Develops code from an EXISTING 系分文档 (produced by prd-to-system-analysis) via
  ...
  any 系分 artifact — defer to prd-to-system-analysis or sdd-dev-agent. Do NOT
  trigger for design-doc / PlantUML — defer to prd-to-system-analysis.
...
Cursor 通过 frontmatter 的 `description` 决定是否激活本 skill。下表是触发与排除的真源（source-of-truth），修改 description 时务必同步此处。本 skill 与上游 `prd-to-system-analysis` 和并列的 `sdd-dev-agent` 共用同一份**互斥分发协议**，三处一致。

### 本 skill 定位

- **输入**：**已有的系分文档**（上游 `prd-to-system-analysis` 产物，至少含 `review.<lang>.md` 或 `system-analysis.<lang>.md` 之一）
- **输出**：代码 / 配置 / 脚本 / 文档改动
...
- 用户只有 PRD、**没有任何系分文档** → 让 [`prd-to-system-analysis`](../prd-to-system-analysis-skill/SKILL.md)（先出系分文档）或 [`sdd-dev-agent`](../sdd_dev_agent/SKILL.md)（跳过系分直出代码）接管
- 用户要 **生成系分 / 设计文档 / PlantUML / 架构图** → 让 `prd-to-system-analysis` 接管
- 用户的"系分文档"其实只是一份 PRD（无 SPLIT/CURRENT_STATE/PLAN/REVIEW 等系分阶段产物）→ 先反问要不要走 `prd-to-system-analysis` 补齐
...
| 仅 PRD + `系分` / `系统分析` / `PlantUML` / `方案设计` / `架构设计` / `概要设计` / `详细设计` / `接口设计` / `A B 方案` / `多系统` / `走流程` / `评审` | `prd-to-system-analysis`（#1） |
| 仅 PRD + `写代码` / `开发` / `实现` + `个人需求` / `小需求` / `小工具` / `内部工具` / `POC` / `原型` / `快速实现` / `不要系分` / `跳过系分` | `sdd-dev-agent`（#3） |
| 仅 PRD + 无场景信号（只说"做一下"/"开始"） | **三方都反问，统一话术见下** |

### Default ask on ambiguity（仅 PRD + 无下游意图时三方一致反问）

> 这个 PRD 你想要：
> - **A. 先生成系分文档**（适合多系统、对齐、正式流程、要架构图）→ `prd-to-system-analysis`
> - **B. 直接生成代码**（适合个人需求、小工具、原型、POC、快速实现）→ `sdd-dev-agent`
> - **C. 已有系分文档要写代码** → `system-analysis-to-sdd-dev`
...
| Prompt | 该路由到 |
|--------|----------|
| "`@PRD.md` 帮我开发个小工具" | `sdd-dev-agent`（无系分 + 个人小工具） |
| "PRD 在这里，帮我直接写代码" | `sdd-dev-agent`（无系分 + 直接写代码） |
| "PRD 来了，先出方案设计 + PlantUML" | `prd-to-system-analysis`（要的是系分文档） |
| "`@PRD-微粒贷.md` 这是多系统改动，请按 lcs/instloancore 分别出系分" | `prd-to-system-analysis` |
| "把这份 PRD 解读一下" | 不触发（无开发意图） |
| "我有一份所谓系分文档其实就是 PRD" | 反问 / 先去 `prd-to-system-analysis` 补齐系分 |
| "`@spec.zh-CN.md` 帮我快速生成代码"（无 ALIGN/dev-* 等系分锚点） | `sdd-dev-agent`（轻量直出代码场景） |
